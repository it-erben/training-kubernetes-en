...
In this exercise you'll create two ServiceAccounts and matching roles in Kubernetes, each with different
access rights to certain resources. You'll then bind the ServiceAccounts to the roles and verify that
the permissions actually took effect.

## The `kubectl auth can-i` Command

kubectl has a built-in way to check whether a principal in Kubernetes has certain permissions.
Here is the documentation: [](https://kubernetes.io/docs/reference/kubectl/generated/kubectl_auth/kubectl_auth_can-i/)
For example, this command checks whether the ServiceAccount "admin" in the default namespace is allowed
to list all pods:
...
- Create a role named `developer` in the default namespace. This role should have read-only access
  (`get`, `list`, `watch`) to the resource types `Pod`, `ReplicaSet`, and `CronJob`.
